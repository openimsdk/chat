...
COMPOSE_FILE="$BASE_DIR/images/openim-chat/docker-compose.build.yml"
...
build_local() {
  echo -e "${CYAN}Building $SERVICE...${NO_COLOR}"
  RELEASE="$RELEASE" docker compose -f "$COMPOSE_FILE" build "$SERVICE"

  built_image=$(docker compose -f "$COMPOSE_FILE" config --images "$SERVICE" 2>/dev/null || true)
  ...
  echo -e "${GREEN}Successfully built $SERVICE${NO_COLOR}"
}
...
build_push() {
  ...
  echo -e "${GREEN}Successfully pushed $SERVICE${NO_COLOR}"
}
