...
func (o *adminServer) AddDefaultGroup(ctx context.Context, req *admin.AddDefaultGroupReq) (*admin.AddDefaultGroupResp, error) {
	...
	// Assuming the CallerInterface is already initialized in the adminServer
	...
	groups, err := o.CallerInterface.FindGroup(ctx, req.GroupIDs)
	if err != nil {
		return nil, err
	}

	missingGroupIDs := utils.Single(req.GroupIDs, utils.Slice(groups, func(group *sdkws.GroupInfo) string { return group.GroupID }))
	if len(missingGroupIDs) > 0 {
		return nil, errs.ErrGroupIDNotFound.Wrap(strings.Join(missingGroupIDs, ", "))
	}
...
func (o *adminServer) SearchDefaultGroup(ctx context.Context, req *admin.SearchDefaultGroupReq) (*admin.SearchDefaultGroupResp, error) {
	...
	groupMap, err := o.CallerInterface.MapGroup(ctx, groupIDs)
	if err != nil {
		return nil, err
	}
