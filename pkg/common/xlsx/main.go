...
func GenXlsx(file *excelize.File, v interface{}) error {
	val := reflect.ValueOf(v)
	if val.Kind() == reflect.Ptr {
		val = val.Elem()
	}
	...
	if len(columns) != len(fieldIndex) {
		return errors.New("struct column repeat")
	}
	file.DeleteSheet(sheetName)
	file.NewSheet(sheetName)

	for x, column := range columns {
		if err := file.SetCellValue(sheetName, GetAxis(x+1, 1), column); err != nil {
			return err
		}
	}
	for i := 0; i < val.Len(); i++ { // y
		item := val.Index(i)
		for x, column := range columns {
			if err := file.SetCellValue(sheetName, GetAxis(x+1, i+2), item.Field(fieldIndex[column]).Interface()); err != nil {
				return err
			}
		}
	}
