...
func InitConfig(configFile string, hide bool) error {
	...
	if err := ComponentCheck(configFile, hide); err != nil {
		return err
	}
...
func newZkClient() (*zk.Conn, error) {
	var c *zk.Conn
	c, _, err := zk.Connect(Config.Zookeeper.ZkAddr, time.Second, zk.WithLogger(log.NewZkLogger()))
	fmt.Println("zk addr=", Config.Zookeeper.ZkAddr)
	if err != nil {
		fmt.Println("zookeeper connect error:", err)
		return nil, errs.Wrap(err)
	} else {
		if Config.Zookeeper.Username != "" && Config.Zookeeper.Password != "" {
			if err := c.AddAuth("digest", []byte(Config.Zookeeper.Username+":"+Config.Zookeeper.Password)); err != nil {
				return nil, errs.Wrap(err)
			}
		}
	}
...
func checkGetCfg(conn *zk.Conn, hide bool) error {
	for i := 0; i < MaxConnectTimes; i++ {
		...
		path := "/" + Config.Zookeeper.Schema + "/" + constant.OpenIMCommonConfigKey
